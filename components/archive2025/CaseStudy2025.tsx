...
import { type Project } from "@/data/archive2025/projects";
import { caseStudies, type CodeFile, type VisualBlock } from "@/data/archive2025/caseStudies";
import { ProjectThumbnail } from "@/components/ProjectThumbnails";
import { projects } from "@/data/archive2025/projects";

// ── Per-case unique components (dynamically imported, ssr:false) ──
const RulerLines        = dynamic(() => import("@/components/archive2025/CaseStudy/RulerLines").then(m => m.RulerLines),                   { ssr: false });
const TokenDepthViz     = dynamic(() => import("@/components/archive2025/CaseStudy/TokenDepthVisualizer").then(m => m.TokenDepthVisualizer), { ssr: false });
const StatsCounter      = dynamic(() => import("@/components/archive2025/CaseStudy/StatsCounter").then(m => m.StatsCounter),               { ssr: false });
const GridAssembly      = dynamic(() => import("@/components/archive2025/CaseStudy/GridAssembly").then(m => m.GridAssembly),               { ssr: false });
const GapLedger         = dynamic(() => import("@/components/archive2025/CaseStudy/GapLedger").then(m => m.GapLedger),                     { ssr: false });
const WarmVignette      = dynamic(() => import("@/components/archive2025/CaseStudy/WarmVignette").then(m => m.WarmVignette),               { ssr: false });
const RecoveryFlowchart = dynamic(() => import("@/components/archive2025/CaseStudy/RecoveryFlowchart").then(m => m.RecoveryFlowchart),     { ssr: false });
const SpecimenGrid      = dynamic(() => import("@/components/archive2025/CaseStudy/SpecimenGrid").then(m => m.SpecimenGrid),               { ssr: false });
const ScriptWeightStrip = dynamic(() => import("@/components/archive2025/CaseStudy/ScriptWeightStrip").then(m => m.ScriptWeightStrip),     { ssr: false });
const WeChatPrimer      = dynamic(() => import("@/components/archive2025/CaseStudy/WeChatPrimer").then(m => m.WeChatPrimer),               { ssr: false });
const BeforeAfterReveal = dynamic(() => import("@/components/archive2025/CaseStudy/BeforeAfterReveal").then(m => m.BeforeAfterReveal),     { ssr: false });
  ...
  return (
    <div onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave} style={{ textAlign: "center", padding: ctaText === "" ? "40px 0 120px" : "100px 0 120px", position: "relative" }}>
      ...
      <motion.div animate={{ x: btnOffset.x, y: btnOffset.y }} transition={{ type: "spring", stiffness: 150, damping: 15, mass: 0.5 }} style={{ display: "inline-block" }}>
        <Link ref={btnRef} href="/archive#contact" onMouseEnter={() => setIsHovered(true)} style={{
      ...
      >
        ...
        <div className="cs-nav-links" style={{ display: "flex", alignItems: "center", gap: 28, height: "100%" }}>
          {(["Work", "Lab", "About", "Contact"] as const).map((label) => (
            <a
              key={label}
              href={`/archive#${label.toLowerCase()}`}
              style={{
      ...
      <div ref={contentRef} style={{ maxWidth: 1160, margin: "0 auto", padding: "0 clamp(24px, 6vw, 96px)" }}>
        ...
        <div style={{ borderTop: "1px solid var(--border)", paddingTop: 40, paddingBottom: 80 }}>
          ...
          <div className="cs-footer-nav" style={{ display: "flex", justifyContent: "space-between", gap: 16 }}>
            {prevProject ? (
              <Link
                href={`/archive/work/${prevProject.id}`}
                style={{
            ...
            {nextProject ? (
              <Link
                href={`/archive/work/${nextProject.id}`}
                style={{
                  fontFamily: sans, fontSize: 16, color: "var(--text-secondary)",
                  textDecoration: "none", display: "flex", flexDirection: "column", gap: 4,
